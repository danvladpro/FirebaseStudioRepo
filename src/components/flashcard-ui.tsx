...
export function FlashcardUI({ challenges }: FlashcardUIProps) {
  ...
  React.useEffect(() => {
    ...
    const onSelect = (api: CarouselApi) => {
      setCurrent(api.selectedScrollSnap() + 1);
      setIsFlipped(false); // Reset flip state on slide change
    };
            ...
            {challenges.map((challenge, index) => (
                <CarouselItem key={index}>
                    <div className="p-1">
                        ...
                        >
                            <Card
                                className={cn("flashcard transition-transform duration-500 w-full min-h-[400px]", isFlipped ? 'is-flipped' : '')}
