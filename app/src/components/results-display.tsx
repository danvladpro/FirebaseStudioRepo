...
import { XP_CONFIG, LEVEL_THRESHOLDS } from './home-page-client';
...
export default function ResultsDisplay() {
  ...
  const { stats, isLoaded, currentLevel: oldLevel } = usePerformanceTracker();
  ...
  useEffect(() => {
    ...
    const updatePerformance = async () => {
      try {
        ...
        // Check for level up
        if (showXp && oldLevel) {
            const currentTotalXP = Object.keys(stats).reduce((acc, currentSetId) => {
                const set = ALL_CHALLENGE_SETS.find(s => s.id === currentSetId);
                if (set?.level && stats[currentSetId]?.bestScore === 100) {
            ...
            setNewTotalXP(newTotalXP);
            
            const newLevelInfo = LEVEL_THRESHOLDS.slice().reverse().find(l => newTotalXP >= l.xp);
            
            if (newLevelInfo && newLevelInfo.level !== oldLevel.level) {
    ...
    updatePerformance();
  }, [isLoaded, setId, time, score, user, mode, oldLevel, showXp, stats, xpEarned]);
